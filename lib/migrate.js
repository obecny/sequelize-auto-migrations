...
const reverseSequelizeColType = function (col, prefix = 'Sequelize.') {
  ...
  switch (attrName) {
    ...
    case DataTypes.STRING.key:
      if (!options.length)
        return prefix + 'TEXT';
      return prefix + 'STRING' + ('(' + options.length + ')') +
...
const reverseSequelizeDefValueType = function (defaultValue, prefix = 'Sequelize.') {
  if (typeof defaultValue === 'object') {
    if (defaultValue.constructor && defaultValue.constructor.name) {
      let value = prefix + defaultValue.constructor.name.toLowerCase();
      if (defaultValue.val) {
        value += '(\'' + defaultValue.val + '\')';
      }
      return { internal: true, value: value };
    }
  }
...
const parseIndex = function (idx) {
  ...
  idx.fields.forEach(function (field) {
...
const reverseModels = function (sequelize, models) {
  ...
  for (let model in models) {
    ...
    if (models[model].options.indexes.length > 0) {
      let idx_out = {};
      const primaryKeys = Object.keys(models[model].primaryKeys);
      for (let _i in models[model].options.indexes) {
        const keys = models[model].options.indexes[_i].fields.map(function (field) {
          return field.name;
        });
        if (primaryKeys.join(',') !== keys.join(',')) {
...
//                    log(df)
      ...
      case 'E': {
        ...
        if (df.path[1] === 'schema') {
          ...
          if (options.references)
            depends.push(options.references.nodel);
          if (df.path[3] === 'onDelete' || df.path[3] === 'onUpdate') {
            const constraintName = `${tableName}_${df.path[2]}_fkey`;
            const newAction = {
              ...
              options: {
                ...
                references: {
                  table: options.references.model,
                  field: options.references.key,
                },
              },
              depends: depends,
            };

            const exists = actions.filter((action)=> {
              return JSON.stringify(action) === JSON.stringify(newAction);
            });
            if (!exists.length) {
              actions.push(newAction);
              actions.push({
                actionType: 'removeConstraint',
                tableName: tableName,
                attributeName: `${tableName}_${df.path[2]}_fkey`,
                options: options,
                depends: depends,
              });
            }
...
const getMigration = function (actions) {
  ...
  for (let _i in actions) {
    ...
    ${propertyToStr(action.options)}
      ...
      case 'addConstraint':
        {
        let res = `{ fn: "addConstraint", params: [
    "${action.tableName}",
    ...
    ${propertyToStr(action.options)}
        ...
        consoleOut.push(`addConstraint "${action.options.name}" on table "${action.tableName}"`);
      }
        break;
...
//                log(action)
    ...
    ${nameOrAttrs}
      ...
      case 'removeConstraint': {
        let nameOrAttrs = (`"${action.attributeName}"`);
        let res = `{ fn: "removeConstraint", params: [
    "${action.tableName}",
    ${nameOrAttrs}
] }`;
        commandsUp.push(res);

        consoleOut.push(`removeConstraint ${nameOrAttrs} from table "${action.tableName}"`);
      }
